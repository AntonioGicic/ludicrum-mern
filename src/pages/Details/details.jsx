...
const Details = () => {
    ...
    const getEventsDetails = async () => {
        ...
        try {
            ...
            if (fullDetails.dateEnd) {
                ...
                setIsDateEnd(fullDetails.dateEnd.slice(0, 10));
            }
            const thisUrl = window.location.href;
            setToFacebook('https://www.facebook.com/share.php?u=' + thisUrl)
            setPublishedDate(fullDetails.createdAt.slice(0, 10));
            setIsDateStart(fullDetails.dateStart.slice(0, 10));
            setEventDetail(fullDetails);
            const id = fullDetails._id;
            const allCookies = eventCookies;
            if (!allCookies.hasOwnProperty(id)) {
                setEventCookie(id, true, { path: '/', maxAge: 1296000 })
            }
        } catch (err) {
            console.error(err);
        }
    ...
    return (
        <>
            {reportActive ? <Report clickToTurnOff={turnOffReport} id={eventDetail} /> : null}
